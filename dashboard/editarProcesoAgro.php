<?php
    ...
    $sentencia = $bd->prepare("UPDATE agropecuario SET Area = ?, Fecha = ?, Objeto = ?, Clave = ?, Descripcion = ?, Responsable = ? where codigo = ?;");
    $resultado = $sentencia->execute([$Area, $Fecha, $Objeto, $Clave, $Descripcion, $Responsable, $codigo]);

    if ($resultado === TRUE) {
        header('Location: agropecuario.php?mensaje=editado');
    } else {
        header('Location: agropecuario.php?mensaje=error');
        exit();
    }
