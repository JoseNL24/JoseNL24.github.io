<?php
    ...
    $sentencia = $bd->prepare("UPDATE informatica SET Area = ?, Fecha = ?, Objeto = ?, Clave = ?, Descripcion = ?, Responsable = ? where codigo = ?;");
    $resultado = $sentencia->execute([$Area, $Fecha, $Objeto, $Clave, $Descripcion, $Responsable, $codigo]);

    if ($resultado === TRUE) {
        header('Location: informatica.php?mensaje=editado');
    } else {
        header('Location: informatica.php?mensaje=error');
        exit();
    }
