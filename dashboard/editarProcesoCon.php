<?php
    ...
    $sentencia = $bd->prepare("UPDATE conciliador SET Area = ?, Fecha = ?, Objeto = ?, Clave = ?, Descripcion = ?, Responsable = ? where codigo = ?;");
    $resultado = $sentencia->execute([$Area, $Fecha, $Objeto, $Clave, $Descripcion, $Responsable, $codigo]);

    if ($resultado === TRUE) {
        header('Location: conciliador.php?mensaje=editado');
    } else {
        header('Location: conciliador.php?mensaje=error');
        exit();
    }
